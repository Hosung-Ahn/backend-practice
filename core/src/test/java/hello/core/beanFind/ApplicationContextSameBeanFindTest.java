...
public class ApplicationContextSameBeanFindTest {
    ...
    @Test
    @DisplayName("조회 시 같은 타입이 둘 이상 있으면 이름을 지정하면 된다.")
    void findBeanByName() {
    ...
    @Test
    @DisplayName("같은 타입을 가지는 모든 빈 조회하기")
    void findAllBeanByType() {
        Map<String, MemberRepository> memberRepositoryMap = ac.getBeansOfType(MemberRepository.class);
        for (String s : memberRepositoryMap.keySet()) {
            System.out.println(s + " = " + memberRepositoryMap.get(s));
        }
    }
